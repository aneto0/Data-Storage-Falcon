...
{
    /**
     * List of terminator that are to be monitored
     */
    String pvTerminators [] = {"-RDI", "-XS", "-XI0", "-XI", "-IN", "-RDO", "-IN", "-YT", "-XA", "-XA0", "-XA", "-XA01", "-XA02", "-XA1", "-XA2", "\0"};
    ...
    public void createSnapshotPulseFile(String xmlVariablesFile, String expName, boolean debug)
    {
        ...
        try {
            ...
            while((currLine = br.readLine()) != null)
            {
                ...
                //Only add variables which terminate with one one of the pvTerminators
                boolean terminatorFound = false;
                for (int k=0; (k<pvTerminators.length) && (!terminatorFound); k++) {
                    terminatorFound = currVarName.endsWith(pvTerminators[k]);
                }
                if (!terminatorFound) {
                    continue;
                }
