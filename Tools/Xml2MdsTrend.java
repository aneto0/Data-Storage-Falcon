        ...
        try {
            ...
            while((currLine = br.readLine()) != null)
            {
                ...
                if (currDescription != null) {
                    ...
                    if (indexOfBracket != -1) {
                        String currDescriptionNoBrackets = currDescription.substring(indexOfBracket).replaceAll("\\s+|]\\s*|\\[\\s*","");
                        String properties[] = currDescriptionNoBrackets.split(",");
                        for(int i=0; (i<properties.length) && (!storeTrend); i++) {
                            storeTrend = (properties[i].trim().equalsIgnoreCase("TREND"));
                        }
                    }
                }
                if (!storeTrend) {
