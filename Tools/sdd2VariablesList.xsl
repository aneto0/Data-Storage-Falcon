...
<xsl:template match="//psic:IandCProject//psic:variables">
<xsl:for-each select = "psic:variable">
   <xsl:sort select = "@name"/>
   <xsl:value-of select = "@name"/>
   <xsl:text> ; </xsl:text>
   <xsl:value-of select = "@description"/>
   <xsl:text> ; </xsl:text>
   <xsl:value-of select = "psic:dataType/psic:name"/>
   <xsl:text> ; </xsl:text>
   <xsl:value-of select = "psic:attributes/psic:attribute/psic:value"/>
   <xsl:text> ; </xsl:text>
   <xsl:value-of select = "psic:implementsTemplate/psic:macroGroupID"/>
   <xsl:text> ; </xsl:text>
   <xsl:value-of select = "psic:direction"/>
   <xsl:text>&#10;</xsl:text>
</xsl:for-each>
</xsl:template>
